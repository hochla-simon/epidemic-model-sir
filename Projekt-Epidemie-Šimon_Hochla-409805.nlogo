...
to destroy-people
  let i 0
  let j birth-death-rate
  
  while [j > 0] [
    
    let oldest-turtles (turtles with [age > max-age - i])
    
    ifelse (count oldest-turtles > 0 )
    [
      ask one-of oldest-turtles
      [
        set turtles-count turtles-count - 1
        die
      ]
      set j j - 1
    ]
...
to create-people
  ...
  create-turtles birth-death-rate [
    setxy random-xcor random-ycor
    set cured? false
    set infected? false
    set susceptible-0 1
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="true">
    ...
    <timeLimit steps="500"/>
    <metric>ticks</metric>
    <enumeratedValueSet variable="birth-death-rate">
      <value value="3"/>
    </enumeratedValueSet>
